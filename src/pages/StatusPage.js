...
export default function StatusPage({ onBack }) {
    ...
    return (
        <div className="h-screen w-screen bg-slate-950 flex relative overflow-hidden select-none animate-fade-in">
            ...
            <div className={`w-full md:w-[400px] flex flex-col bg-slate-900/60 backdrop-blur-3xl border-r border-white/5 z-50 overflow-hidden ${selectedGroupIdx !== null ? 'hidden md:flex' : 'flex'} relative shadow-[10px_0_30px_rgba(0,0,0,0.3)]`}>
                ...
                <div className="p-6 border-b border-white/5 flex items-center justify-between relative z-10">
                    <div>
                        <h2 className="text-white font-black text-2xl tracking-tight">Status</h2>
                        <p className="text-white/40 text-xs font-bold uppercase tracking-widest mt-1">Updates & Stories</p>
                    </div>
                    <button onClick={onBack} title="Back to Chats" className="p-3 bg-white/5 hover:bg-white/10 rounded-xl transition-all text-white/60 hover:text-white hover:scale-105 active:scale-95 border border-white/5">
                ...
                <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-3 custom-scrollbar relative z-10">
                    {/* MY STATUS SECTION */}
                    <div className="px-2 py-2 text-white/40 text-[10px] uppercase font-bold tracking-widest flex items-center gap-2">
                        <div className="h-px bg-white/10 flex-1"></div>
                        <span>My Status</span>
                        <div className="h-px bg-white/10 flex-1"></div>
                    </div>

                    <div className="flex items-center gap-4 p-4 rounded-3xl transition-all mb-2 bg-gradient-to-r from-white/5 to-transparent hover:from-white/10 border border-white/5 group relative overflow-hidden">
                        ...
                        <div className="relative cursor-pointer" onClick={() => fileInputRef.current?.click()}>
                            ...
                            <div className="w-16 h-16 rounded-full p-[3px] bg-gradient-to-tr from-primary to-blue-400 shadow-[0_0_15px_rgba(59,130,246,0.3)] group-hover:shadow-[0_0_25px_rgba(59,130,246,0.5)] transition-all duration-500">
                                <div className="w-full h-full rounded-full border-4 border-slate-900 overflow-hidden bg-slate-800 relative">
                                    {user?.avatar ? <img src={user.avatar} alt={user.full_name} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-white/50 font-bold text-lg">{user?.full_name?.[0]}</div>}
                                    <div className="absolute inset-0 bg-black/20 group-hover:bg-transparent transition-colors" />
                                </div>
                            </div>
                            <div className="absolute bottom-0 right-0 w-6 h-6 bg-primary rounded-full flex items-center justify-center text-white border-2 border-slate-900 text-lg font-bold shadow-lg transform group-hover:scale-110 transition-transform">+</div>
                        </div>
                        ...
                        <div className="flex-1 flex items-center justify-between relative z-10">
                            <div className="cursor-pointer" onClick={() => {
                                ...
                                <div className="text-white/40 text-xs font-medium mt-0.5">{myGroup ? `${myGroup.statuses.length} updates` : 'Tap to add status update'}</div>
                            </div>
                            <button onClick={() => fileInputRef.current?.click()} className="p-3 bg-white/5 hover:bg-white/20 text-white/40 hover:text-white transition-all rounded-xl hover:scale-105 active:scale-95">
                    ...
                    {otherGroups.length > 0 && (
                        <>
                            <div className="px-2 py-4 text-white/40 text-[10px] uppercase font-bold tracking-widest flex items-center gap-2">
                                <div className="h-px bg-white/10 flex-1"></div>
                                    ...
                                    return (
                                        <div
                                            ...
                                            className={`flex items-center gap-4 p-4 rounded-3xl cursor-pointer transition-all animate-slide-up group border border-transparent ${globalIdx === selectedGroupIdx
                                                ? 'bg-primary/20 border-primary/20 shadow-[0_0_20px_rgba(59,130,246,0.15)]'
                                        ...
                                        >
                                            <div className={`w-14 h-14 rounded-full p-[2px] transition-all duration-500 ${hasUnviewed
                                                ...
                                                <div className="w-full h-full rounded-full border-4 border-slate-900 overflow-hidden bg-slate-800">
                                                    {g.user.avatar ? <img src={g.user.avatar} alt={g.user.full_name} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-white/50 font-bold">{g.user.full_name?.[0]}</div>}
                                                </div>
                                            </div>
                                            <div className="flex-1 overflow-hidden">
                                                <div className={`text-white font-bold text-base truncate transition-colors ${hasUnviewed ? 'text-white' : 'text-white/60'} group-hover:text-primary`}>
                                                    {g.user.full_name}
                                                </div>
                                                <div className="text-white/40 text-xs truncate flex items-center gap-2 mt-0.5">
                                                    <span className={`w-1.5 h-1.5 rounded-full ${hasUnviewed ? 'bg-emerald-500 animate-pulse' : 'bg-white/20'}`}></span>
                                                    {new Date(g.statuses[0].createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                                                </div>
                                            </div>
                                            <svg className={`w-5 h-5 text-white/20 group-hover:text-white/60 transition-all ${globalIdx === selectedGroupIdx ? 'opacity-100 text-primary' : 'opacity-0 group-hover:opacity-100'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M9 5l7 7-7 7" /></svg>
                                        </div>
                                    );
                                })}
                            </div>
            ...
            <div className={`flex-1 flex flex-col relative h-full bg-black ${selectedGroupIdx === null ? 'hidden md:flex' : 'flex'}`}>
                {currentStatus ? (
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center text-center p-6 relative overflow-hidden">
                        ...
                        <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-primary/20 rounded-full blur-[150px] animate-pulse-slow pointer-events-none" />
                        <div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-purple-500/10 rounded-full blur-[150px] animate-pulse-slow animation-delay-2000 pointer-events-none" />

                        <div className="w-32 h-32 bg-gradient-to-tr from-slate-900 to-slate-800 rounded-[3rem] flex items-center justify-center backdrop-blur-md border border-white/10 mb-10 shadow-[0_20px_50px_rgba(0,0,0,0.5)] relative z-10 animate-scale-up">
                            <div className="absolute inset-0 bg-gradient-to-tr from-primary/20 to-transparent rounded-[3rem]" />
                            <svg className="w-16 h-16 text-primary drop-shadow-[0_0_15px_rgba(59,130,246,0.5)]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3 9a2 2 0 012-2h.93a2 2 0 001.664-.89l.812-1.22A2 2 0 0110.07 4h3.86a2 2 0 011.664.89l.812 1.22A2 2 0 0018.07 7H19a2 2 0 012 2v9a2 2 0 01-2 2H5a2 2 0 01-2-2V9z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M15 13a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
                        </div>

                        <h3 className="text-white text-5xl font-black tracking-tighter mb-4 relative z-10 drop-shadow-2xl">
                            Status <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-blue-400">Stories</span>
                        </h3>

                        <p className="text-white/60 text-lg max-w-[400px] mx-auto leading-relaxed mb-12 relative z-10 font-medium">
                        ...
                        <div className="flex flex-col sm:flex-row gap-5 relative z-10">
                            <button
                                ...
                                className="px-10 py-5 bg-gradient-to-r from-primary to-blue-600 text-white font-bold rounded-2xl hover:to-primary transition-all hover:scale-105 shadow-[0_10px_30px_rgba(37,99,235,0.4)] flex items-center gap-3 group border border-white/10"
                            >
                            ...
                            {statusGroups.length === 0 && (
                                <button onClick={onBack} className="px-10 py-5 bg-white/5 text-white/60 hover:text-white font-bold rounded-2xl hover:bg-white/10 transition-all border border-white/5 hover:border-white/20">
                                    Return to Chats
                                </button>
                            )}
                        </div>
            ...
            {uploading && (
                <div className="absolute inset-0 bg-slate-950/90 z-[200] flex flex-col items-center justify-center backdrop-blur-2xl animate-fade-in">
                    <div className="relative">
                        <div className="w-20 h-20 border-4 border-white/10 rounded-full" />
                        <div className="absolute inset-0 border-4 border-primary border-t-transparent animate-spin rounded-full shadow-[0_0_30px_rgba(59,130,246,0.6)]" />
                    </div>
                    <span className="text-white font-black tracking-[0.3em] text-sm uppercase mt-8 animate-pulse text-center">Uploading Moment...</span>
                </div>
            )}
        </div>
    );
